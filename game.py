...
def draw_menu():
    ...
    screen.fill('black') # type: ignore
    screen.draw.text("UFO Jumper", center=(WIDTH/2, HEIGHT/3), fontsize=80, color="white") # type: ignore
    screen.draw.text("Pressione [ENTER] para Iniciar", center=(WIDTH/2, HEIGHT/2), fontsize=40, color="white") # type: ignore
    screen.draw.text("Pressione [M] para Mutar/Desmutar Som", center=(WIDTH/2, HEIGHT/2 + 60), fontsize=30, color="white") # type: ignore
    screen.draw.text("Pressione [ESC] para Sair", center=(WIDTH/2, HEIGHT/2 + 110), fontsize=30, color="white") # type: ignore
    screen.draw.text("Use as setas laterais para se mover, use o SPACE 2x para o pulo duplo, capture a bandeira", center=(WIDTH/2, HEIGHT/2 + 150), fontsize=20, color="white") # type: ignore
...
def draw_game_elements():
    ...
    if player.alive:
        player.draw()
    if over:
        screen.draw.text("Game Over!", center=(WIDTH/2, HEIGHT/2), fontsize=60, color="white") # type: ignore
    if win:
        screen.draw.text("You Win!", center=(WIDTH/2, HEIGHT/2), fontsize=60, color="white") # type: ignore
...
def player_die():
    ...
    music.stop() # type: ignore
    if sound_enabled:
        sounds.gameover.play() # type: ignore
...
def player_win(flag):
    ...
    music.stop() # type: ignore
    if sound_enabled:
        sounds.gamewin.play() # type: ignore
    flags.remove(flag)
...
def on_key_down(key):
    ...
    if not game_started:
        if key == keys.RETURN: # type: ignore
            game_started = True
            play_music()
        elif key == keys.M: # type: ignore
            sound_enabled = not sound_enabled
            if sound_enabled:
                play_music()
            else:
                music.stop() # type: ignore
        elif key == keys.ESCAPE: # type: ignore
            exit()
    else:
        if key == keys.SPACE and player.jumping > 0: # type: ignore
            if sound_enabled:
